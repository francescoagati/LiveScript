...
exports.compile = function(code, options){
  var result, ast, output, js, filename, outputFilename, ref$, mapPath, e, that;
  ...
  try {
    if (options.json) {
      ...
      if (options['es6-class']) {
        js = toString$.call(output).slice(8, -1) === 'String'
          ? output
          : output.toString();
        output = exports.transpileEs6Class(js);
      }
...
exports.transpileEs6Class = function(code){
  var m, _, name, body;
  m = code.match(/^var (\w+);\n\1 = \(function\(\)\{\n([\s\S]*?)\n\}\(\)\);?$/);
  if (!m) {
    return code;
  }
  _ = m[0], name = m[1], body = m[2];
  body = body.replace(new RegExp("^\\s*" + name + "\\.displayName[^\n]*\n"), '').replace(new RegExp("\n\\s*" + name + "\\.displayName[^\n]*\n"), "\n").replace(/^\s*var prototype = [^\n]+\n/, '').replace(/\n\s*var prototype = [^\n]+\n/, "\n").replace(new RegExp(name + "\\.prototype\\.(\\w+) = function\\(", 'g'), function(__, m){
    return m + "(";
  }).replace(/};\n/g, "}\n").replace(new RegExp("function " + name + "\\("), 'constructor(').replace(new RegExp("return " + name + ";"), '');
  return "class " + name + " {\n" + body + "\n}";
};
