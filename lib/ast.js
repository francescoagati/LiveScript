...
exports.Block = Block = (function(superclass){
  ...
  Block.prototype.compileRoot = function(options){
    var o, that, ref$, bare, prefix, ref1$, ref2$, comment, ref3$, code, result;
    o = (import$({
      level: LEVEL_TOP,
...
exports.Class = Class = (function(superclass){
  ...
  Class.prototype.compile = function(o, level){
    ...
    if (o['es6-class']) {
      return this.compileEs6(o, level);
    }
  ...
  Class.prototype.compileEs6 = function(o, level){
    var fun, body, lines, title, name, ind, inner, ctor, props, i$, len$, node, parts, methO, ref$, fn, code, prop, key;
    fun = this.fun, body = fun.body, lines = body.lines, title = this.title;
    name = (title != null ? title.varName() : void 8) || this.name;
    ind = o.indent;
    inner = ind + TAB;
    ctor = null;
    props = [];
    for (i$ = 0, len$ = lines.length; i$ < len$; ++i$) {
      node = lines[i$];
      if (node instanceof Fun && !node.statement) {
        ctor = node;
      } else if (node instanceof Obj) {
        props.push.apply(props, node.items);
      }
    }
    parts = ["class ", name];
    if (this.sup) {
      parts.push(" extends ", this.sup.compile(o, LEVEL_PAREN));
    }
    parts.push(" {\n");
    methO = (ref$ = {}, import$(ref$, o), ref$.indent = inner, ref$.level = LEVEL_TOP, ref$);
    if (ctor) {
      ctor.name = undefined;
      fn = ctor.compile(methO);
      code = fn.toString().replace(/^function\s*\w*\s*/, '');
      parts.push(inner, "constructor", code, "\n");
    }
    for (i$ = 0, len$ = props.length; i$ < len$; ++i$) {
      prop = props[i$];
      fun = prop.val;
      key = prop.key instanceof Key
        ? prop.key.name
        : prop.key.compile(o);
      fn = fun.compile(methO);
      code = fn.toString().replace(/^function\s*\w*\s*/, '');
      parts.push(inner, key, code, "\n");
    }
    parts.push(ind, "}");
    return sn.apply(null, [null].concat(arrayFrom$(parts)));
  };
  return Class;
}(Node));
